package org.easy.ecm.security.auth;

import java.io.Serializable;
import java.security.Principal;

/**
 * <p>
 * Basic implementation of the Principal class. By implementing our own
 * Principal for our application, we can more easily add and remove
 * instances of our principals in the authenticated Subject during the
 * login and logout process.
 *
 * @see java.security.Principal
 * @author  
 * @version 1.0
 */
public class EcmPrincipal implements Principal, Serializable{

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;


    private String name;

    /**
     * Create a <code>EcmPrincipal</code> with no
     * user name.
     *
     */
    public EcmPrincipal() {
        name = "";
    }

    /**
     * Create a <code>EcmPrincipal</code> using a
     * <code>String</code> representation of the
     * user name.
     *
     * <p>
     *
     * @param name the user identification number (UID) for this user.
     *
     */
    public EcmPrincipal(String newName) {
        name = newName;
    }

    /**
     * Compares the specified Object with this
     * <code>EcmPrincipal</code>
     * for equality.  Returns true if the given object is also a
     * <code>EcmPrincipal</code> and the two
     * EcmPrincipal have the same name.
     *
     * <p>
     *
     * @param o Object to be compared for equality with this
     *		<code>EcmPrincipal</code>.
     *
     * @return true if the specified Object is equal equal to this
     *		<code>EcmPrincipal</code>.
     */
    public boolean equals(Object o) {

        if (o == null)
            return false;

        if (this == o)
            return true;
 
        if (o instanceof EcmPrincipal) {
            if (((EcmPrincipal) o).getName().equals(name))
                return true;
            else
                return false;
        }
        else 
            return false;
    }

    /**
     * Return a hash code for this <code>EcmPrincipal</code>.
     *
     * <p>
     *
     * @return a hash code for this <code>EcmPrincipal</code>.
     */
    public int hashCode() {
        return name.hashCode();
    }

    /**
     * Return a string representation of this
     * <code>EcmPrincipal</code>.
     *
     * <p>
     *
     * @return a string representation of this
     *		<code>EcmPrincipal</code>.
     */
    public String toString() {
        return name;
    }

    /**
     * Return the user name for this
     * <code>EcmPrincipal</code>.
     *
     * <p>
     *
     * @return the user name for this
     *		<code>EcmPrincipal</code>
     */
    public String getName() {
        return name;
    }

}
